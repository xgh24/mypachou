...
class MovieTop(object):
    ...
    def upload(self):
        db = pymysql.connect("localhost", "root", "15056425990", "jgh", charset='utf8')
        cursor = db.cursor()
        cursor.execute('DELETE FROM web_movie')

        insertStr = "INSERT INTO web_movie(name, director,Country)VALUES ('%s','%s','%s')"

        try:
            for movie in self.movieList:
                insertSQL = insertStr % ( str(movie[1]), str(movie[3]),str(movie[5]))
